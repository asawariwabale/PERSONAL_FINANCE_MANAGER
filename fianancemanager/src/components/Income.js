...
function Income() {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      console.log("User ID not found in stored user");
    }
  }, []);

  // Fetch incomes when userId available
  useEffect(() => {
    if (userId !== null) {
      fetchIncome();
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:8086/income/add", newIncome);
      setAmount("");
      setSource("");
      setDate("");
      fetchIncome();
  ...
  return (
    <>
      ...
      <div className="container mt-4">
        <h4 className="mb-3">Add Income</h4>
